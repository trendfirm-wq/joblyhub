...
const approveJob = async (req, res) => {
  try {
    ...
    // A job can be approved if it was paid for
    // OR if the employer has been granted free posting access.
    const isFreePostingEmployer =
      job.employer?.canPostFree === true;

    if (job.paymentStatus !== 'paid' && !isFreePostingEmployer) {
      return res.status(400).json({
    ...
    // If this is a free-posting employer, make sure
    // the job is recorded as paid/exempt internally.
    if (isFreePostingEmployer && job.paymentStatus !== 'paid') {
